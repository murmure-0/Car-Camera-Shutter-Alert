 ...
 */
static inline __attribute__((unused)) int get_list_num(const struct list_head* head)
{
	struct list_head* p;
	p = head->next;
    int num =0;
 ...
 * param:   head  The list item to be printed.
 */
static inline __attribute__((unused)) void print_list(struct list_head* head)
{
	struct list_head* p;
	p = head->next;
	while (p != head)
 ...
 */
static inline __attribute__((unused)) void clear_list(struct list_head* l) {
	struct list_head* p;
    if(l==NULL){
        return;
    }
